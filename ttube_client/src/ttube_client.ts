...
const attributeProps = <T>(e: Element, props: (PropName<T> | PropMap<T>)[]) => {
    ...
    return mapToObj(nameValues, ({ name, value }) => [name, value]) as any as T
}

const chartRender: { [prop: string]: (e: Element) => ReactElement } = {
    electionFraud: _ => el(NarrativeVideoComponent, {}),
    vaccineChannel: _ => el(NarrativeHighlightComponent, {}),
    vaccineVideo: _ => el(NarrativeVideoComponent, {}),
